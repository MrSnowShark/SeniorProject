...
class FlightController extends Controller
{
    ...
    public function loadReplay(Request $request)
    {	
		$flight = Request::route('flight');
        $validFlight = $this->validateFlight();
        $path = 'tmp/';
...
HDR;
    ...
    {	
		$flight = Request::route('flight');
		$data = DB::table('main')->select('latitude', 'longitude', 'msl_altitude', 'heading', 'pitch_attitude', 'roll_attitude')->where('flight', $flight)->get();
		...
		foreach($data as $datum)
		{
			...
			$flightStr .= "$datum->heading,";
			$flightStr .= "$datum->pitch_attitude,";
			$flightStr .= "$datum->roll_attitude,";
			$flightStr .= " ";
		}
		
		//$json = json_encode($flightStr)
        return view('flights/replay')->withData($flightStr);
    }
